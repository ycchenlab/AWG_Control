...
# This Python file generates the .txt file containing the information needed for the AOD conrol program to generate the
# required waveforms.
# The required information in the file contains the sampling rate for the AWG, the number of distinct frequency
# components,the relative amplitude and relative phase between each frequency components, the starting and ending
# frequencies for each step, and the duration of each frequency shift step.
#
# The txt file for setup should contain the following information in the following order:
# Line 1 of the .txt file: Sampling frequency in MHz.
# For this board (M4i.6622-x8), 50 MHz < sampling frequency < 625 MHz
#
# The subsequent lines are grouped into sets of 3 lines.
# The first and second lines contain two sets of frequencies of equal number along with their relative amplitudes and
# phases.
...
# Line 1: This line contains number of frequencies, frequency components (in MHz), relative amplitude, relative phase
# (in radian)
...
lattice_spacing = 2  # unit: MHz, denotes the frequency spacing between neighboring coordinate points (min: 0.45 MHz in Lukin's paper)
...
# only for movement process, no turn-off
...
# Generate the frequency change and write in the files
